...
import { formatCurrency, generateId, getNextColor } from "@/lib/storage";
...
export default function CategoryPanel({ categories, expenses, onAddCategory, onDeleteCategory }: CategoryPanelProps) {
  ...
  const handleAdd = () => {
    ...
    onAddCategory({ id: generateId(), name: newName.trim(), color: getNextColor(categories) });
    setNewName("");
    setIsAdding(false);
  };
              ...
              <div className="flex items-center gap-3">
                ...
                {deleteConfirm === cat.id ? (
                  <div className="flex items-center gap-1">
                    <button onClick={() => handleDelete(cat.id)} className="text-[10px] px-2 py-0.5 rounded bg-red-500/15 text-red-400 hover:bg-red-500/25 transition-colors font-[Space_Grotesk] font-bold tracking-wider">DELETE</button>
                  ...
                  <button onClick={() => handleDelete(cat.id)} className="opacity-0 group-hover:opacity-100 text-[#45464d] hover:text-red-400 transition-all">
          ...
          <div className="px-5 py-3.5 border-t border-white/5">
            <div className="flex items-center gap-2">
              <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)}
                ...
                autoFocus />
              <button onClick={handleAdd} className="px-3 py-1.5 text-[10px] font-bold bg-[#4edea3]/15 text-[#4edea3] rounded hover:bg-[#4edea3]/25 transition-colors font-[Space_Grotesk] tracking-wider">ADD</button>
            </div>
          </div>
        ) : (
